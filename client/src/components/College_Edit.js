...
export default function College_Edit() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await fetch(`http://localhost:3000/recordColleges/${params.id.toString()}`);
    ...
    <div> 
      <form onSubmit={onSubmit}>
        <div className="form-group" >
          <label style={{ marginTop: 220 ,marginLeft:228 }} htmlFor="College_Name">College Name</label>
          <input 
        ...
        <div className="form-group">
          ...
          />
        </div>      <Navbar />  <LeftSide_College />  <CenterCollege /> <Bottom />
      </form>
    </div>
  );
}